...
Built-in tool implementations. **Prefer extensions for new tools** — this dir exists only for tools that ship in upstream `pi-mono` and need parity. The helix-specific extras (`apply_patch`, web search, code execution, computer use) are builtin extensions, not core tools.
...
- **Tools live in core only when upstream parity demands it**. Helix-specific tools (`apply_patch`, web search, etc.) live as builtin extensions in `extensions/builtin/`.

## ANTI-PATTERNS

- Editing `bash.ts` `promptSnippet` to re-introduce `grep` — already fixed; helix has a dedicated `grep` tool.
