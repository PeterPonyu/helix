...
LLMs have limited context windows. When conversations grow too long, helix uses compaction to summarize older content while preserving recent work. This page covers both auto-compaction and branch summarization.
...
For TypeScript definitions in your project, inspect `node_modules/@helix-bio/helix/dist/`.
...
By default, `reserveTokens` is 16384 tokens (configurable in `~/.helix/agent/settings.json` or `<project-dir>/.helix/settings.json`). This leaves room for the LLM's response.

You can also trigger manually with `/compact [instructions]`, where optional instructions focus the summary.

### How It Works

1. **Find cut point**: Walk backwards from newest message, accumulating token estimates until `keepRecentTokens` (default 20k, configurable in `~/.helix/agent/settings.json` or `<project-dir>/.helix/settings.json`) is reached
...
For split turns, helix generates two summaries and merges them:
...
When you use `/tree` to navigate to a different branch, helix offers to summarize the work you're leaving. This injects context from the left branch into the new branch.
...
Both compaction and branch summarization track files cumulatively. When generating a summary, helix extracts file operations from:
...
Configure compaction in `~/.helix/agent/settings.json` or `<project-dir>/.helix/settings.json`:
