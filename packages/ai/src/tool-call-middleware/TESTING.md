...
Your `~/.helix/agent/models.json` must include the following provider configurations:
...
**Test Command:**
```bash
helix --provider openrouter-qwen --model "qwen/qwen3.5-27b" -p "Read the contents of package.json and tell me the project name"
```
...
**Test Command:**
```bash
helix --provider openrouter-gemini --model "google/gemini-3-flash-preview" -p "List all TypeScript files in src/ using the find tool"
```
...
**Test Command:**
```bash
helix --provider openrouter-gemma --model "google/gemma-4-31b-it" -p "Use bash to run 'date' and tell me the current time"
```
...
```bash
DEBUG=pi:* helix --provider openrouter-qwen --model "qwen/qwen3.5-27b" -p "Read package.json"
```
...
```bash
helix --provider <provider> --model <model> --info
```
...
|----------|----------|-------|--------------|
| Hermes | openrouter-qwen | qwen/qwen3.5-27b | `helix --provider openrouter-qwen --model "qwen/qwen3.5-27b" -p "Read package.json"` |
| MorphXml | openrouter-gemini | google/gemini-3-flash-preview | `helix --provider openrouter-gemini --model "google/gemini-3-flash-preview" -p "List TypeScript files"` |
| Gemma4 | openrouter-gemma | google/gemma-4-31b-it | `helix --provider openrouter-gemma --model "google/gemma-4-31b-it" -p "Run date command"` |
